...
//! ```
//!
//! Now we can create the loop! The easiest way to create a loop is using [`Loop::init()`](crate::tickloop::Loop::init)
//!
//! ```rust
...
//! ```
//!
//! Now that you have a loop all that is left is to send it events. To do this you can use the event_sender that was returned by [`Loop::init()`](crate::tickloop::Loop::init). The event sender is a `Sender<Event<EventType>>` where EventType is the type of event you specified in your listener. **EventType is wrapped in a [`Event`](crate::event::Event)**. This is to guarentee that you can send a close event to the loop. To send an event you can use the [`send()`](std::sync::mpsc::Sender::send()) method on the event sender.
//!
